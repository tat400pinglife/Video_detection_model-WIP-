...
class DeepfakeDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return rgb, diff, fft, label

# --- 2. THE MODEL ARCHITECTURE ---
# Copy and paste this over the existing SimpleBranch and TinyDeepfakeDetector classes
...
class TinyDeepfakeDetector(nn.Module):
    def __init__(self):
        ...
        self.classifier = nn.Sequential(
            # 65,536 * 3 branches = 196,608 inputs
    ...
    def forward(self, rgb, diff, fft):
        ...
        combined = torch.cat([f_rgb, f_diff, f_fft], dim=1)
        return self.classifier(combined)

# --- 3. TRAINING LOOP ---
def train_one_epoch():
    # Ensure this points to where you saved your .pt files
    # Previous script used "data/frames/real" etc, so root is "data/frames"
    ...
    for epoch in range(1, 10):
        ...
        model.train()
        for rgb, diff, fft, labels in loader:
            optimizer.zero_grad()
            
            # FIX: Use .view(-1) to handle batch size 1 correctly
            outputs = model(rgb, diff, fft).view(-1)
