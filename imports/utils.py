...
def compute_features(frames, video_path, device=None):
    """
    Calculates ALL 5 forensic features: RGB, RGB-Seq, PRNU, FFT, Diff, Audio
    """
    ...
    # Grayscale conversion (Standard Rec. 601)
    ...
    # --- B. PRNU (Noise Residual) ---
    prnu_stack = []
    subset = gray_stack[max(0, mid_idx-2):min(len(gray_stack), mid_idx+3)]
    for g in subset:
    ...
    t_diff  = torch.from_numpy(diff_map).unsqueeze(0).unsqueeze(0).float().to(device)
